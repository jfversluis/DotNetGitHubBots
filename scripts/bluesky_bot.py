...
"""
Bluesky GitHub Bot
Fetches new issues/PRs from multiple GitHub repositories and posts them to Bluesky.
Uses a tracking issue in the bot repository for persistence.
Supports multiple repository groups with different Bluesky accounts.
"""
...
def get_tracking_issue(bot_repo_owner, bot_repo_name, github_token, bot_group_name):
    ...
    tracking_title = f"Bluesky Bot State - {bot_group_name}"
...
def post_to_bluesky(bluesky_client, issue, repo_full_name):
...
def process_bot_group(bot_config, bot_repo_owner, bot_repo_name, github_token):
    """Process one bot group (multiple repos posting to one Bluesky account)."""
    ...
    bluesky_handle = get_env_var(bot_config['bluesky']['handle_secret'])
    bluesky_app_password = get_env_var(bot_config['bluesky']['password_secret'])
    ...
    for repo_config in bot_config['repositories']:
        ...
        if new_issues:
            ...
            for issue in new_issues:
                if post_to_bluesky(client, issue, repo_full_name):
...
def main():
    ...
    print("Starting Bluesky GitHub Bot (Multi-Repository)")
    print("=" * 60)
